...
final class SensorMonitor: ObservableObject {
    ...
    @Published var magnetometerAccuracy: Int = 0
    ...
    func start() {
        ...
        // Start device motion (combines all sensors)
        if motionManager.isDeviceMotionAvailable {
            motionManager.startDeviceMotionUpdates(to: .main) { [weak self] motion, error in
                ...
                self.magnetometerX = motion.magneticField.field.x
                self.magnetometerY = motion.magneticField.field.y
                self.magnetometerZ = motion.magneticField.field.z
